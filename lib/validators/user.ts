...
export const completeClientProfileSchema = z.object({
  ...
  businessName: z.string().trim().min(1, "Business name is required.").max(100, "Business name must not exceed 100 characters."),
});
...
export const completeFreelancerProfileSchema = z.object({
  ...
  phone: z
    ...
    .regex(/^\+?[0-9\s\-()]{7,20}$/, "Invalid phone number format."),
  location: z.string().trim().min(1, "Location is required.").max(100, "Location must not exceed 100 characters."),
  bio: z.string().trim().min(10, "Bio must be at least 10 characters.").max(500, "Bio must not exceed 500 characters."),
});
